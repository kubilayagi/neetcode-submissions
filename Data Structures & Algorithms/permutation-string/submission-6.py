    ...
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        for r in range(len(s1), len(s2)):
            ...
            if s2counts[rindex] == s1counts[rindex]:
                ...
                # note: we only need to subtract from the match
                # when the difference is 1 because it means we messed up
                # an existing match. if the difference is not 1, then it 
                # was not a match previously, and we did not mess it up
                # since it was already not counted as a match
                matches -= 1
